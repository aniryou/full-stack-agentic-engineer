...
# `decode_aggregate` leaves out: the batch per GPU is capped by HBM and by the ITL SLO.
...
# %% [markdown]
# Ten times the output tokens needs eighteen times the GPUs for memory: concurrency grows 10× (Little's law) and
# each live session holds 1.8× the KV (3,000 average context vs 1,650). The decode *throughput* need grows only
# 10× and is not the binding constraint. With a 20 ms ITL SLO the step time caps each GPU at 187 sessions, below
# the 209 that fit in HBM: ITL binds first.
#
...
# `per_gpu` (sessions per GPU at the average context 1,500 + 3,000/2) and `gpus` = live / per_gpu.
...
print(f"✅ {live:,.0f} live sessions / {per_gpu:.1f} per GPU = {gpus:.2f} GPUs — vs 0.26 without thinking")
...
# capped by HBM and by the ITL SLO, which binds first when it is tight. We size max_model_len for the tail and
