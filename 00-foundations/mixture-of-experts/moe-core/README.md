...
deployment — with `moecore`, a standard-library + numpy package small enough to read in a sitting (~970 lines).

## Start here

1. Read [`../PRIMER.md`](../PRIMER.md) "The one-minute version", then §1–§2 (why sparsity, the MoE layer).
2. `python3 -m pip install -r requirements.txt && python3 -m pytest -q` — 61 tests in about 7 s, including "the
...
|---|---|---|---|---|
| [`01_the_moe_layer`](notebooks/01_the_moe_layer.ipynb) | route tokens through five real routers (Mixtral, Qwen3, DeepSeek-V3, gpt-oss, Llama 4) and say why their weights differ; write the sparse forward pass; count total and active parameters from a config and name the "active" convention a published number uses; argue for fine-grained experts | §1, §2 | ~1.5 h | T0 |
...
| [`03_which_experts_a_batch_touches`](notebooks/03_which_experts_a_batch_touches.ipynb) | derive E(1 − (1 − k/E)^T); reproduce layer 01's MoE table; predict the decode crossover from total ÷ active; explain rows per expert (B·k/E) and why MoE wants big batches; show what skew does (simulated) | §5 | ~1.5 h | T0 |
| [`04_expert_parallelism_and_all_to_all`](notebooks/04_expert_parallelism_and_all_to_all.ipynb) | price dispatch and combine on NVLink, PCIe and InfiniBand; explain DeepEP's published numbers; find the slowest rank under skew and rebalance it; compare TP and EP; choose a wide-EP degree | §6 | ~1.5 h | T0 |
| [`05_sizing_and_cost`](notebooks/05_sizing_and_cost.ipynb) | size memory by total, prefill by active, decode by bytes streamed; plan GPUs and EP degree against an ITL target; cost per million tokens against a dense model; price CPU offload on a small GPU | §7, §8 | ~1 h | T0 |
...
python3 -m pytest -q                           # 61 tests, ~7 s
...
|------|------:|-----------------|
| [`moecore/moe.py`](moecore/moe.py) | ~270 | the layer: `Expert` (SwiGLU), `route()` with the five families' routers (`ROUTERS`), group-limited routing, `MoELayer.forward` (sort by expert → one GEMM per expert → weighted scatter-add) and `forward_dense` (the reference); `MoEConfig` counts total and active parameters (three embedding conventions) and KV bytes per token |
...
| [`moecore/ep.py`](moecore/ep.py) | ~150 | expert parallelism: dispatch bytes (FP8 scales included), α-β all-to-all, placement, the exchange matrix, the slowest rank, EPLB-style rebalancing, TP vs EP, wide-EP memory, a decode step on n GPUs in the EP or TP layout |
| [`moecore/sizing.py`](moecore/sizing.py) | ~130 | `MODELS` (Mixtral, Qwen3, Qwen1.5-MoE, DeepSeek-V3, gpt-oss, Llama 4, OLMoE, granite, and dense Llama 3.1 for comparison; dated, verify-marked), weight bytes with quantized experts, sessions that fit, prefill FLOPs, the all-experts floor, `plan()`, cost per million tokens, CPU-offload penalty |

## What the tests prove

`tests/` has one focused test per concept (61, offline, ~7 s). The ones that carry the claims:
...
no overlap of communication with compute; MLA attention FLOPs approximated; prices are placeholders. The trainer is a
...
Go to [`../moe-lab/`](../moe-lab/) to train a tiny MoE in torch, watch a real router with hooks, measure decode step
