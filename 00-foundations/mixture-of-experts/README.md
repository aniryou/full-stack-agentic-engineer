...
2. `cd moe-core && python3 -m pip install -r requirements.txt && python3 -m pytest -q` — 61 tests, ~7 s; then open
   [`01_the_moe_layer`](moe-core/notebooks/01_the_moe_layer.ipynb).
3. With torch installed (CPU is enough), train a tiny MoE in the lab, [`moe-lab/`](moe-lab/): notebook
   `01_a_tiny_moe_in_torch`; with any GPU, `02_watch_the_router` records a real router's choices.
...
| [`moe-lab/`](moe-lab/) | **measure**: package `moelab` — a tiny MoE in torch, router hooks on real MoE models, decode step time vs batch in vLLM, `--enable-expert-parallel` on two GPUs, offload and 4-bit experts on a 16–24 GB GPU; `deploy/any-gpu/` and GKE manifests for the 02 lab's `l4x2` pool. Every notebook falls back to a labelled T0 path | ~6 h | T0 → T3 |
...
|---|---|---|---|
| 1 | [PRIMER §1–2](PRIMER.md#1-why-sparsity) why sparsity, the layer, counting parameters | `01_the_moe_layer` | `01_a_tiny_moe_in_torch` — T0 with torch |
| 2 | [PRIMER §3–4](PRIMER.md#3-routing-and-load-balance) balance, collapse, training | `02_routing_and_load_balance` | `02_watch_the_router` — T1 (bundled traces at T0) |
| 3 | [PRIMER §5](PRIMER.md#5-moe-at-inference-which-experts-a-step-touches) experts touched, the crossover, big batches | `03_which_experts_a_batch_touches` | `03_batch_vs_weight_stream` — T1 (simulated at T0) |
| 4 | [PRIMER §6](PRIMER.md#6-running-moe-on-gpus) fused kernels, EP, wide-EP, offload | `04_expert_parallelism_and_all_to_all` | `04_expert_parallelism_on_two_gpus` — T2 on Kaggle 2×T4 (simulated at T0) |
| 5 | [PRIMER §7–9](PRIMER.md#7-sizing-and-cost) sizing, cost, failure modes, where to run | `05_sizing_and_cost` | `05_moe_on_a_small_gpu` — T1 (sizing at T0) |
...
Quantizing experts connects to the quantization topic in `04-inference-engine/quantization/`.
...
- **The toy trainer is a toy.** Four linear experts on a clustered regression: the direction of its results
  (collapse without balancing, balance with it) holds across seeds; the exact numbers do not transfer.
