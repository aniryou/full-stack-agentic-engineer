...
kubectl apply -f deploy/gpu-vm/20-time-sliced.yaml && kubectl get pods -o wide
kubectl logs -l job-name=time-sliced            # four pods, one GPU UUID: shared, not isolated
...
* **Time-slicing is sharing without isolation.** All four pods print the same GPU UUID; nothing
  limits one pod's memory. A container that requests `nvidia.com/gpu: 2` can receive two slices
  of the *same* GPU; the plugin's `failRequestsGreaterThanOne` option rejects that instead.
