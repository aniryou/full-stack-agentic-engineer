...
def test_primer_numbers_are_the_simulators_numbers():
    ...
    bundle = make_cluster(hosts=1)                                     # 3.4's CPU/memory-bundle stranding
    s = Scheduler(bundle)
    s.submit(*[gpu_pod(f"c{i}", 1, cpu=16000) for i in range(8)])
    s.run()
    host = next(iter(bundle.nodes.values()))
    assert (len(host.pods), host.free(GPU), stranded_gpus(bundle, 1)) == (6, 2, 0)   # free mod k misses them
    assert "768 GiB / 64 GiB) = **6** per node" in PRIMER
