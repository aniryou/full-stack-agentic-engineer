...
**223 notebooks**, all runnable in Google Colab with one click.
...
| **`02-cuda-nccl-runtime/`** | Why a kernel is fast or slow, what a collective costs, how a container gets a GPU, how a GPU is shared and monitored | `cuda-and-nccl/`: a primer (the driver-to-framework stack, the execution model, memory access patterns, launch overhead and CUDA Graphs, collectives and NCCL, how a container gets a GPU, MIG vs MPS vs time-slicing, DCGM and XIDs), `cuda-nccl-core/` (seven numpy simulators that compute every worked number), `cuda-nccl-lab/` (Numba CUDA kernels in the simulator then on a GPU, collectives measured like nccl-tests with an α-β fit, what a container sees of its GPU, DCGM alert rules; any GPU box, Kaggle 2 × T4, GKE via Terraform) |
