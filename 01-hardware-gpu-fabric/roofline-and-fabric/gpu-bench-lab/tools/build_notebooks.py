...
kept). A bootstrap cell makes ``import gpubench`` work from a fresh checkout.
"""
...
BOOTSTRAP = '''# bootstrap: Colab clone + local import of `gpubench` (auto-inserted)
import sys, pathlib
if "google.colab" in sys.modules:
    ...
    os.chdir(_repo / "01-hardware-gpu-fabric/roofline-and-fabric/gpu-bench-lab")
    subprocess.run([sys.executable, "-m", "pip", "install", "-q", "-e", "."])
_r = pathlib.Path.cwd().resolve()
while _r != _r.parent and not (_r / "gpubench").exists():
    _r = _r.parent
...
def build(path: Path):
    ...
    for variant, out_dir in (("exercise", EX), ("solution", SOL)):
        ...
        for i, cell in enumerate(nb.cells):   # stable ids: rebuilding unchanged sources leaves git clean
            cell.id = hashlib.sha1(f"{path.stem}:{variant}:{i}".encode()).hexdigest()[:8]
