...
# powers off, so a broken run never idles on the GPU until max_run_duration. A VM restarted after a
# finished run finds $WORK/.done and powers off again without re-running.
#
...
FINISHED=0
power_off() { # stop the VM (GPU billing stops) unless metadata gpubench-poweroff says "false"
...
if [ -f "$WORK/.done" ] && [ "$DRY_RUN" != "1" ]; then
  # A VM started again after a finished run (gcloud compute instances start) must not idle on the
  # GPU until max_run_duration: power off again, the same way a finished run does.
  step "already ran on this disk ($WORK/.done); delete it to run again (set gpubench-poweroff=false to keep the VM up)"
  power_off
  exit 0
fi
run mkdir -p "$OUT"
