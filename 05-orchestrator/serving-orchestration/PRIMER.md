...
The 3:2:2 weights are not an arbitrary example: they are the default profile the llm-d-router Helm chart installs
(`default-plugins.yaml` in v0.10.0, when its latency predictor is off) `(verify)`, and the lab's `default-weighted`
preset reproduces them. A default is a starting point, not a tuning: §2.5 shows the optimum moving with the workload.
...
The optimized-baseline rows trail the 3:2:2 chart default on this workload, and the reason is what their gate can see. With
...
**The ranking itself is not portable.** It flips with where the engine contends and with the workload. The lab
([`inference-gateway-lab`](inference-gateway-lab/), notebook
...
(`llm-d.ai/v1alpha2`; `spec.poolRef`, `spec.priority` as an int32) sets the priority for a workload on a pool:
negative values are allowed, and an unset priority counts as 0. What the number does depends on the `flowControl`
feature gate:

- **Gate on (flow control):** requests queue in the router by priority band, and a higher band is always served
  first. Fairness is enforced only *within* a band (the default ordering is global FCFS). This is the behaviour the
  resource's API documentation describes and `fleetsim.FlowControl` models (the table above).
- **Gate off — the llm-d-router v0.10.0 default:** there is no router queue and no ordering. The legacy admission
  check only marks negative-priority objectives as *sheddable*: while the saturation detector reports the pool
  saturated, their requests are rejected with HTTP 429, and every request at priority 0 or above is admitted and
  routed as usual. Strict priority ordering needs the gate on `(verify)`. The lab's router implements this legacy
  path (`inference-gateway-lab`, `igwlab/router/server.py`).

Priority matters only when requests queue — exactly when the pool is saturated — so it is the tool for "the
interactive agent before the nightly eval" (in `fleetsim`, `Request.priority`). With the gate off, the only lever
is to give the nightly eval a negative priority, so that it is refused at saturation instead of delayed.
