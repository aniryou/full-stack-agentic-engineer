...
# into 16-token blocks. So an 8B model in bf16 on an L4 has about 4-5 GiB of KV: ~37K tokens, which
# means the default 128K context does not even start, and at 8K context only four requests fit in
# the worst case. FP8 weights plus FP8 KV more than doubles that (smaller weights *and* half-size KV).
