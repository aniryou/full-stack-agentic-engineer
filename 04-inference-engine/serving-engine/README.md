...
| 04.7 Parallelism, LoRA, measurement, deployment | §9 Parallelism inside the engine · §10 Multi-LoRA serving · §11 Measuring an engine · §12 Engines and where to run them | [`01`](mini-engine-core/notebooks/01_the_step_loop_and_continuous_batching.ipynb) exercise 1.6 (tensor parallelism); [`02`](mini-engine-core/notebooks/02_chunked_prefill_and_the_token_budget.ipynb) worked example 3 (open loop vs saturation, goodput) | [`03_knobs_and_tradeoffs`](vllm-serving-lab/notebooks/03_knobs_and_tradeoffs.ipynb), [`06_deploy_on_cloud_run_gpu`](vllm-serving-lab/notebooks/06_deploy_on_cloud_run_gpu.ipynb) | T0 → T1 (T3) |
