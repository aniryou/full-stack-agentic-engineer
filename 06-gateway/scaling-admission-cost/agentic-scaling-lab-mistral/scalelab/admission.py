...
The controller looks at four signals — turns in flight, the share of recent model calls
that were pushed back (429 from the API, 503 from a capped vLLM queue), how long the oldest
queued turn has waited, and how saturated the model backend is (KV-cache use plus queue on a
self-hosted fleet; the same thresholds the Kubernetes inference gateway uses) — and maps them
to a degrade level:
...
class AdmissionConfig:
    max_inflight: int = 175              # from the token budget or the fleet: see capacity.plan()
    soft_ratio: float = 0.8              # level 1 above this share of the cap
    rate_limited_degrade: float = 0.05   # level 1 at 5 % pushback (429/503), level 2 at 15 %
    queue_age_degrade_s: float = 10.0
    queue_age_shed_s: float = 30.0
    saturation_degrade: float = 0.8      # level 1 at 80 % backend saturation, level 2 once a queue exists (≥ 1.0)
    dwell_s: float = 15.0
...
class AdmissionController:
    ...
    # --- the level ------------------------------------------------------------------
    def compute_level(self, queue_age_s: float = 0.0, circuit_open: bool = False, saturation: float = 0.0) -> int:
        c, ratio = self.cfg, self.rate_limited_ratio()
        raw = 0
        if (self.inflight >= c.max_inflight * c.soft_ratio or ratio >= c.rate_limited_degrade
                or queue_age_s >= c.queue_age_degrade_s or saturation >= c.saturation_degrade):
            raw = 1
        if ratio >= 3 * c.rate_limited_degrade or circuit_open or saturation >= max(1.0, c.saturation_degrade):
            raw = 2
    ...
    # --- admit / release --------------------------------------------------------------
    def admit(self, priority: int = 5, queue_age_s: float = 0.0, circuit_open: bool = False, saturation: float = 0.0) -> Decision:
        level = self.compute_level(queue_age_s, circuit_open, saturation)
