...
file (`agentsec_core.py`, ~250 lines of code), one dependency (`PyJWT[crypto]`), and the five
...
| 2. Authority | act under the agent's OWN authority or one DELEGATED by a user: a token naming both (`sub` = user, `act` = agent), for ONE audience, narrow scope, 5-minute life; an agent can never widen a user's grant | `Issuer`, `Authority` | Auth Manager / STS (RFC 8693 token exchange) |
...
pytest -q                        # 10 one-sentence-each tests
jupyter lab core_walkthrough.ipynb
```

`core_walkthrough.ipynb` is the worked version; `core_practice.ipynb` has 20 blanks with
