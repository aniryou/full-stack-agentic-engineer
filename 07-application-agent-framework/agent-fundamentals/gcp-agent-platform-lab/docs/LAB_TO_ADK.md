...
transfer. This is the translation table to use when you rebuild a notebook on ADK, or when a
reader uses ADK vocabulary. ADK moves quickly: `google-adk` 2.0.0 reached PyPI on 2026-05-19 and
2.10.0 was current on 2026-09-25 (verify). The ADK and Google Cloud names in the right-hand
columns — Agent Runtime (formerly Agent Engine), Memory Bank, Agent Registry, Agent Gateway,
Agent Identity, the `gemini-3-flash` model string — are as of September 2026 (verify): treat them
as "the name to look up", not gospel, and check the current docs before relying on them.
...
| Deterministic workflows | `SequentialAgent`, `ParallelAgent`, `LoopAgent(max_iterations, until)` | `SequentialAgent`, `ParallelAgent`, `LoopAgent(max_iterations)`; exit via escalate/conditions; native DAG `Workflow` in 2.x (verify) | ADK's loop exits when a sub-agent escalates; here `until(session)` is code |
...
| Budgets | `agents.Budget(max_steps, max_tokens, max_seconds, max_depth)` shared through `InvocationContext` | `RunConfig` limits (e.g. max LLM calls), `RetryConfig` in 2.x (verify) | |
...
| Model | `llm.FakeLLM` / `llm.GeminiLLM` | model string (`gemini-3-flash`, September 2026, verify) or a `Gemini(...)` model object; LiteLLM for others | |
| Dev loop | `make lab`, `tools/run_notebooks.py` | `adk web`, `adk run`, `adk api_server`, `adk eval`, `adk deploy` | |
| Deployment | — (notebook 07 sketches the API) | Agent Runtime (formerly Agent Engine; verify), Cloud Run, GKE | |
...
3. Replace `LlmAgent(..., llm=FakeLLM(...))` with `Agent(model="gemini-3-flash", ...)` (or the current Flash model string; verify); keep the instruction and `output_key`.
