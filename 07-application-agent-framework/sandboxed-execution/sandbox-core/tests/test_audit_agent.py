...
from sandboxcore import (AuditEvent, AuditLog, ProcessSandbox, SandboxAgent, SandboxPolicy,
...
def test_audit_event_has_identity_lab_fields_plus_sandbox_fields():
    ...
    for f in ("budgets_used", "exit_reason", "policy_decision", "isolation"):
        assert f in d
...
def test_injection_exfiltration_is_denied_before_running():
    pol = SandboxPolicy(egress_allowlist=())   # attacker.example is not allowed
    ag = SandboxAgent(injection_scenarios(pol)["exfiltrate"], pol)
...
def test_audit_counts_are_a_kill_reason_histogram():
    log = AuditLog()
    log.record(AuditEvent(event_type="sandbox.result", agent="a", exit_reason="cpu_time"))
    log.record(AuditEvent(event_type="sandbox.result", agent="a", exit_reason="cpu_time"))
